                    ...
                    {products?.map((product, index) => (
                      <tr key={product.id}>
                        ...
                        <td className={`py-2 pl-8 pr-8 ${index !== products.length - 1 ? 'border-b border-gray-300' : ''}`}>
                          {product.isDeleted ? (
                            <span product_id={product.id} role='img' aria-label='Delete' className='delete-icon'>
                              ❌
                            </span>
                          ) : (
                            <span product_id={product.id} role='img' aria-label='Delete' className='text-green-500'>
                              ✔
                            </span>
                          )}
                        </td>
                        <td className={`py-2 pl-8 pr-8 ${index !== products.length - 1 ? 'border-b border-gray-300' : ''}`}>
                          {product.isDeleted ? (
                            <button onClick={() => prueba(product.id)} className='bg-red-200 rounded-xl'>
                              Cambiar Estado
                            </button>
                          ) : (
                            <button onClick={() => prueba(product.id)} className='bg-green-200 rounded-xl'>
                              Cambiar Estado
                            </button>
                          )}
                        </td>
