...
import {
  getCreations,
  resetfilters,
  getPublicacionesFilter,
  getPublicacionesFilters,
  getPublicacionesFilterPrice,
  pagePublications,
  getallcomponents,
} from "../Redux/actions";
...
function Publicaciones() {
  ...
  return (
    <div className="flex flex-col items-center justify-start w-full md:px-20">
      <h3 className="text-gray-900 font-bold text-4xl mt-8 mb-2 text-center">
        Explora y conoce nuestra comunidad
      </h3>
      ...
      <div className="flex md:flex-row flex-col flex-wrap items-center justify-center">
        <button
          ...
          className="bg-orange-600 w-28 md:w-24 h-12 text-white rounded-xl font-bold mt-4 mr-3"
        >
          Todas
        </button>
        <form onSubmit={handlesubmit} className="flex flex-wrap justify-center">
          <select
            ...
            className="bg-gray-300 w-32 md:w-40 h-12 text-black rounded-xl font-bold mt-4 mr-3"
          ...
          <select
            ...
            defaultValue="DEFAULT"
            name="Rating"
            className="bg-gray-300 w-32 md:w-36 h-12 text-black rounded-xl font-bold mt-4 mr-3"
          >
            <option value="DEFAULT" disabled className="text-center">
              Rating
            </option>
          ...
          <button
            type="submit"
            className="bg-orange-600 w-24 md:w-24 h-12 text-white rounded-xl font-bold mt-4 mr-3"
          >
            Filtrar
          </button>
        </form>
        <button
          onClick={handlereset}
          className="bg-orange-500 w-24 md:w-24 h-12 text-white rounded-xl font-bold mt-4"
        >
          Reset
        </button>
      </div>

      <div className="flex flex-wrap justify-center mt-4 gap-2 bg-orange-100 p-2 ">
        {allcomponents &&
          allcomponents.map((elem) => {
            return (
              <>
      ...
      {allCreations.length > 0 && (
        <Pagination action={pagePublications} page={page} maxPage={maxPage} />
      )}
    </div>
  );
}
